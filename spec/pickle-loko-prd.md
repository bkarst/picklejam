...
### 9.1 Principles
- **Capacity: on-demand only (hard requirement).** **Every DynamoDB table — and every GSI — MUST use `PAY_PER_REQUEST` (on-demand) billing; NEVER provisioned throughput.** This applies to all environments (Development/Test/Production) and all future tables/migrations. Enforced in provisioning (`scripts/provision-table.mjs` sets `BillingMode: PAY_PER_REQUEST`); a provisioned-capacity table is a bug.
